...
このリポジトリは、**Webマーケティング学習プロジェクト**の一環です。美容院向け通訳サービス「HairTalk Translator」の擬似ランディングページを[GitHub Pages](https://pages.github.com/)で公開するために作成しました。
...
開発中はブラウザで `index.html` を直接開くより、簡易Webサーバーを使用します。これは相対パスやフォーム送信処理の正常動作を確保するためです。

#### 推奨方法: VSCode Live Server 拡張

1. VSCode の拡張機能から **Live Server** をインストール
2. `index.html` を右クリックして「Open with Live Server」を選択
3. ブラウザが自動起動し、`http://127.0.0.1:5500` などのURLで表示される
...
* フォームデータは JSON 形式で送信、CORS プレフライトリクエストへの対応
* Apps Script では **バリデーション後にスプレッドシート保存** → JSONレスポンスで結果返却
* 成功時はサンクス画面を表示し、**PDFを即時ダウンロード**できる導線を用意
...
* フォームで取得したメールアドレスは学習目的に限定して使用する
* ページ内およびPDFに「学習用である」ことを明記する
* 個人情報の保存期間・削除方法を README またはページ内に記載する